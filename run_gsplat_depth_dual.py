...
def main(local_rank: int, world_rank, world_size: int, cfg: Config):
    ...
    runner = RunnerDual(local_rank, world_rank, world_size, cfg)
...
if __name__ == "__main__":
    configs = {
        "default": (
            "Depth-warmup + dual gsplat training. "
            "Use --depth-warmup-steps > 0 to enable the initial depth phase.",
            Config(strategy=DefaultStrategy(verbose=True)),
        ),
        "mcmc": (
            "Depth-warmup + dual gsplat training with MCMC densification strategy.",
            Config(
                init_opa=0.5,
                init_scale=0.1,
                opacity_reg=0.01,
